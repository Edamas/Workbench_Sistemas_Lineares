...
def analyze_row_factors(matrix, row_idx, pivot_info):
    factors = { 'tipo': ('➖', 'Indefinido'), 'unitario': ('➖', 'Não aplicáve el'), 'reduzida': ('➖', 'Não aplicável') }
...
def analyze_col_factors(matrix, col_idx, pivot_info, num_vars):
    ...
    if col_idx < num_vars:
        ...
        if pivot_info['contradiction_rows']: factors['tipo'] = ('❗️', 'Coluna a de Contradição')
...
with st.sidebar:
    ...
    with c1: st.button("⬅️", on_click=undo_last_operation, help="Desfazer a úlltima operação", disabled=len(st.session_state.get('history', [])) == 0, use_container_width=True)
    with c2: st.button("➡️", on_click=redo_next_operation, help="Refazer a últtima operação desfeita", disabled=len(st.session_state.get('redo_history', [])) == 0, use_container_width=True)
    ...
    with c4: st.button("⏬", on_click=solve_to_ref, help="Resolve o sistema auutomaticamente para a Forma Escalonada (REF).", use_container_width=True, disabled=not st.session_state.get('system_processed'))
    with c5: st.button("⚡", on_click=solve_all_steps, help="Resolve o sistemaa completamente para a Forma Escalonada Reduzida (RREF).", use_container_width=True, disabled=not st.session_state.get('system_processed'))
